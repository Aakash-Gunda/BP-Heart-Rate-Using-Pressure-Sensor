...
Original file is located at
    https://colab.research.google.com/drive/15jBbO02avygvWiuXWtCzdpOXTQivgfHr

# Real Time Embedded Systems (ECE-GY 6483)
## Embedded Challenge Spring 2021

### Introduction
In this notebook, I will calculate the Heart rate, Systolic and Diastolic blood pressure from the cuff pressure readings.
 I am using the STM32 Discovery board, Honeywell MPRLS0300YG00001BB pressure sensor and a blood pressure cuff. The readings
  are taken once every 20ms. The pressure is raised to 200mmHg and is decreased gradually while we get all the useful data
  from 150-80. I am reading this data from the serial from the python in Vscode instead of copying the data from serial 
  print. This gives me some flexibility in the algorithm to extract the Systolic and Diastolic blood pressures. The rest of 
  this code could have be done in Vscode Colab give us a better representation so I have divided this into two codes.
...
Data is submitted in the zip file for reference. I have collected the raw data from serial and applied the transfer function
 in the code below. Since the pressure reading is taken manually I am considering 500 samples after pressure reaches a
  maximum value which has all the data we need for calculating the required values.
"""
...
In this part of the code, we take the Focus_data detrend the signal to get the oscillometric waveform and plot the graph.
 In data detrending, the linear trend is removed along the axis from data ("scipy.signal.detrend") giving us a clean 
 waveform with constant frequency. Here, we also get the peaks as we use them to calculate the heart rate. 
"""
...
Here, we calculate Systolic and Diastolic blood pressure by taking the Mean Arterial Pressure (MAP) from OMW waveform.
 Systolic being 50% of MAP frequency and diastolic being 80% of MAP. We then plot them on the acquired pressure data.
"""
...
We then calculate the heart rate in beats per minute. From using the timer using vscode, I can see that we get a sample
 every 23.57ms (20ms intended) because of the delay caused by other operations. I took 5 peaks and the distance between
  them and multiplied by 60 to calculate the number of beats per minute.
"""
